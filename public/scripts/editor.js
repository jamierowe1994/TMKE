...
import { createResizeEngine } from "./resize-engine.js";
...
(function () {
  ...
  // ---------- Resize ----------
  /* The maths lives in resize-engine.js so the admin size checker runs the
     same code a member does — see the note at the top of that file. */
  const _RESIZE = createResizeEngine({
    textLineCount: function (el, size) { return textLineCount(el, size); },
  });
  const resizePage = _RESIZE.resizePage;
  const resizeElementDetail = _RESIZE.resizeElementDetail;
