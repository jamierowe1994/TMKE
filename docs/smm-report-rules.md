...
**Audience**
Follower activity, gender split, and location data. There is no Audience tab in the client
report and there is not going to be one: most of it is operational detail they have no use
for. A client does not need to know what time we post.

It is background context, with one exception. Where the audience data supports a
*conclusion that matters to their business*, that conclusion may be stated in the Key
Takeaways or Trends prose. The test is whether it tells them something about their market
rather than about our scheduling:

- Permitted: "Your followers are overwhelmingly local to your patch, and the age profile
  skews to the ages most likely to be buying, so the audience we're building is the right
  one."
- Not permitted: posting times, peak-activity heatmaps, raw gender splits, anything that
  reads as us showing our workings.

Never cite it as data. State the conclusion, not the figures behind it.

**Actions**
The social media manager's internal action log for the period, written with the team in
mind. This is the primary source for the Key Takeaways section, but the language
...
**Trends**
Only present when two or more months of data exists, since a trend needs two points to
draw. When present, it provides pattern-level insights across the reporting period. When
absent, the Trends component of the output is not generated.
...
| Key Takeaways          | Actions           | Overview to confirm the numbers behind each action     |
...
Note on where Trends lives: the month-on-month **graph** is reproduced on the Overview
tab, not the Summary, because it is a chart and it belongs with the numbers. The AI does
not draw it. What the AI does do is **acknowledge the trend in words** in the Summary
prose and, where it warrants it, in Key Takeaways. The client sees the shape on Overview
and reads what it means on Summary.
...
4. Let the Trends acknowledgement and Key Takeaways add depth rather than repeat what
...
The graph itself is not here. It is reproduced on the Overview tab with the rest of the
numbers. This component is the words that go with it: what the shape on that graph
actually means. Do not describe the chart ("the line goes up"); say what it tells them.

It covers three things in short, readable form: not headers, not a table, just a few
clear points written in plain English.
...
2. The generator returns a draft summary, a trends acknowledgement where applicable, and
